...
import { jobs, education, projects } from '../curriculum-vitae/cvData';

// Only change name of company for data where company "Hessel de Jong Webdesign"
const anonymizedJobs = jobs.map(job => ({
  ...job,
  company: job.company === 'Hessel de Jong Webdesign' ? '[Geanonimiseerd Bedrijf]' : job.company,
}));

const anonymizedEducation = education;
const anonymizedProjects = projects;
...
const collectAllTags = () => {
  ...
  // Process job tags
  anonymizedJobs.forEach(job => {
    processTags(job.tags);
  });

  // // Process project tags
  // anonymizedProjects.forEach(project => {
  ...
  return (
      <div className="max-w-max mx-auto p-8 my-16 grid md:grid-cols-cv grid-cols-cv-mobile bg-blob bg-no-repeat">
        <section className="my-4 content-end md:text-end mb-2 col-span-2 col-start-1 mr-8">
          <h2 className="text-xl font-bold">[Geanonimiseerde naam]</h2>
          <ul className="list-none my-3 text-base">
            <li className={"text-base mb-1"}>z5hwszxhyv@privaterelay.appleid.com</li>
        ...
        <Education education={anonymizedEducation}/>
        ...
        <Projects projects={anonymizedProjects}/>
